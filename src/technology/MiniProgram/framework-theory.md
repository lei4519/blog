...
说起改变的数据，可能第一反应是diff，我们将老的数据保存一份，当数据改变之后。对新旧数据进行diff，然后拿到差异数据，传入setData更新视图。
...
转换起来那么难，那我干脆不转了。JSX最后生成的是什么？vnode啊，我直接把vnode传给setData喽。
...
看过vue的实现之后，我们不难猜到，这里要动刀的肯定是renderer这一步了。
