...
首先通过 diff 找出本次提交中要处理的文件列表，依次进行处理  
...
通过 metadata 生成博客首页数据（README.md），其实就是根据时间排序的文章链接列表  
  
生成之后就可以开始 jekyll 部署的流程了  
  
#### Github Readme  
  
上面生成的 README.md 不能直接作为 GitHub 的 README.md 提交，因为链接指向不对，我希望 GitHub 里的链接指向 issue 中  
